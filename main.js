...
// Klucz API będzie pobierany z globalnej zmiennej ustawionej w index.html
// const API_KEY = window.APP_API_KEY; // Ta linia zostanie ustawiona przez index.html

let API_KEY = null; // Zmienna do przechowywania klucza API po załadowaniu

// Funkcja inicjalizująca aplikację po załadowaniu klucza API
function initializeApp() {
    API_KEY = window.APP_API_KEY; // Pobierz klucz API
    if (!API_KEY) {
        // To nie powinno się zdarzyć, jeśli flow w index.html jest poprawny
        document.getElementById('statusLabel').textContent = "Błąd: Klucz API nie został przekazany.";
        return;
    }
    console.log("Aplikacja uruchomiona z kluczem API.");

    // Elementy DOM (teraz już są dostępne, bo HTML jest wstrzyknięty)
    ...
    // Po załadowaniu skryptu i ustawieniu API_KEY, sprawdź czy jest zapisany ostatni ID filmu
    const storedVideoId = localStorage.getItem('lastVideoId');
    if (storedVideoId) {
        videoInput.value = storedVideoId;
        // Opcjonalnie: auto-load po załadowaniu apki i wpisaniu API
        // loadVideo();
    }
}

// Uruchom funkcję inicjalizującą po załadowaniu skryptu main.js
// Oczekuje, że index.html ustawi window.APP_API_KEY przed załadowaniem main.js
initializeApp();